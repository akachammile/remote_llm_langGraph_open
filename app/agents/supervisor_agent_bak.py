    ...
    async def top_level_supervisor(self, state: AgentState) -> AgentState:  # type: ignore
        ...
        if response.tool_calls:
            for tool in response.tool_calls:
                ...
                if tool_name == "planning_tool":
                    ...
                    logger.info(f"规划结果为: {type(plan_results)}")
                    state["sub_task"].append(plan_results)                    
                # 图像处理工具
                elif tool_name == "image_segmentation":
                    segmentation_tool = self.available_tools.get_tool(
                        "image_segmentation"
                    )
                    params = {
                    ...
                    segmented_bytes, segmentation_info, save_path = (
                        segmentation_tool.execute(**params)
                    )
                    state["processed_image_path"].append(save_path)
                    logger.info(state["sub_task"])
                ...
                # 文件处理工具
                    ...
                    print("调用了文件处理工具")
                    # segmentation_tool = self.available_tools.get_tool("file_process")
                    # params = {
                    #     "image_data": state["image_data"],
                    #     "image_path": state["image_path"],
                    #     "state": state,
        ...
        if state["sub_task"]:
            ...
            for step in tasks:
                ...
                logger.info(f"🧩 第 {step_id}步: {action} (Tool: {tool_name})")

                tool_name = self.available_tools.get_tool(tool_name)
